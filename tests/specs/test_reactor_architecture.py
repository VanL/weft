...
def test_task_reactor_has_one_broker_wait_gateway() -> None:
    """Task drivers and synchronous probes enter only the shared watcher wait."""
    ...
    for path in _runtime_python_files():
        ...
        for node in ast.walk(tree):
            ...
            if chain is None or not chain.endswith("wait_for_activity"):
                continue
            observed.add((str(path.relative_to(REPO_ROOT)), chain))

    assert observed == {
        ("weft/core/control_probe.py", "watcher.wait_for_activity"),
        ("weft/core/tasks/base.py", "self.wait_for_activity"),
        (
