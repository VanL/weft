...
| `WEFT_TASK_MONITOR_CONTROL_QUEUE_DELETE_LIMIT` | Operator cap for supervised runtime-queue cleanup from task-local `T{tid}.ctrl_in`, `T{tid}.ctrl_out`, and eligible stale `T{tid}.reserved` queues. The monitor may apply a smaller internal fair-slice cap and catch up through `WEFT_TASK_MONITOR_CATCHUP_INTERVAL_SECONDS`. Defaults to 1000. |
