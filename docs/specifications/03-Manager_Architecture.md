...
7. **Managed services** – The manager reconciles autostarts, heartbeat, and
   ...
   mappings, or a matched keyed `PING`/`PONG`; task-owned terminal proof for
   that TID (terminal envelope or terminal task-log status) wins over host
   ...
   sends kill control to non-canonical live owners, force-reaping their
   recorded host process tree when host PID evidence is available, so the
   system converges back to one singleton. Manager `task_spawned` rows for
   service children are valid live owner evidence when their recorded child PID
   is still live. The manager
   does not scan task-log history for cleanup and does not run monitor
   processors itself.
