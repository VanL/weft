...
- **Submission-time TaskSpec materialization**: `weft/core/taskspec/parameterization.py` — declared long-option parsing for `spec.parameterization`, adapter invocation, JSON-serializable payload checks, and local concrete-TaskSpec materialization; `weft/commands/run.py` — shared `weft run` materialization before run-input shaping and spawn submission; `weft/commands/specs.py` — explicit adapter-ref validation.
- **Submission-time run-input shaping**: `weft/core/taskspec/run_input.py` — declared long-option parsing, adapter invocation, JSON-serializable payload checks; `weft/commands/run.py` — shared `weft run --spec` local adapter execution after materialization and before spawn submission; `weft/commands/specs.py` — explicit adapter-ref validation.
