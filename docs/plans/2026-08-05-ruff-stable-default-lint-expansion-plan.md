...
| MR-159 | `weft/commands/_task_snapshot_reducer.py::<module>` | `PYI025=1`; `PYI025@14` | The annotation denotes the built-in mutable set interface; runtime behavior is unaffected. | Replace collections.abc.Set with built-in set and run mypy plus reducer tests. | `./.venv/bin/python -m pytest -q tests/commands/test_task_snapshot_reducer.py && ./.venv/bin/mypy weft --config-file pyproject.toml` | ready: mechanical typing candidate |
